...
// dump value
func printKeyValue(buf *bytes.Buffer, val reflect.Value, pointers **pointerInfo, interfaces *[]reflect.Value, structFilter func(string, string) bool, formatOutput bool, indent string, level int) { // NOSONAR
	t := val.Kind()
...
func PrintPointerInfo(buf *bytes.Buffer, headlen int, pointers *pointerInfo) { // NOSONAR
	anyused := false
	pointerNum := 0
