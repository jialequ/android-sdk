...
func BuildTemplate(dir string, files ...string) error { // NOSONAR
	var err error
	fs := beeTemplateFS()
	f, err := fs.Open(dir)
	if err != nil {
	...
	for _, v := range self.files {
		for _, file := range v {
			if buildAllFiles || utils.InSlice(file, files) {
				...
				if len(ext) == 0 {
					t, err = getTemplateString(self.root, fs, file, v...)
...
func getTplDeep(root string, fs http.FileSystem, file string, parent string, t *template.Template) (*template.Template, [][]string, error) { // NOSONAR
...
func getTemplateString(root string, fs http.FileSystem, file string, others ...string) (t *template.Template, err error) {
	...
	t, err = getTemplate(t, root, fs, subMods, others...)
...
func getTemplate(t0 *template.Template, root string, fs http.FileSystem, subMods [][]string, others ...string) (t *template.Template, err error) { // NOSONAR
	t = t0
