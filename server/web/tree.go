...
func (t *Tree) addtree(segments []string, tree *Tree, wildcards []string, reg string) { // NOSONAR
	if len(segments) == 0 {
...
func filterTreeWithPrefix(t *Tree, wildcards []string, reg string) { // NOSONAR
...
// "/"
// "admin" ->
func (t *Tree) addseg(segments []string, route interface{}, wildcards []string, reg string) { // NOSONAR
	if len(segments) == 0 {
		if reg != "" {
...
func (t *Tree) match(treePattern string, pattern string, wildcardValues []string, ctx *context.Context) (runObject interface{}) { // NOSONAR
	if len(pattern) > 0 {
		i, l := 0, len(pattern)
		for i < l && pattern[i] == '/' {
			i++
...
func (leaf *leafInfo) match(treePattern string, wildcardValues []string, ctx *context.Context) (ok bool) { // NOSONAR
...
func splitSegment(key string) (bool, []string, string) { // NOSONAR
